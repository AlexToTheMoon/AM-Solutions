## Remote Signing setup via TMKMS for Qwoyn-1 mainnet.
...
```
mkdir -p $HOME/tmkms/qwoyn
tmkms init $HOME/tmkms/qwoyn
```
#### Import Private key
...
```
tmkms softsign import $HOME/priv_validator_key.json $HOME/tmkms/qwoyn/secrets/qwoyn-consensus.key
```
...
```
rm -rf ~/tmkms/qwoyn/tmkms.toml
```
```
tee ~/tmkms/qwoyn/tmkms.toml << EOF
#Tendermint KMS configuration file
[[chain]]
id = "qwoyn-1"
key_format = { type = "bech32", account_key_prefix = "qwoynpub", consensus_key_prefix = "qwoynvalcons" }
state_file = "$HOME/tmkms/qwoyn/state/qwoyn-1_priv_validator_state.json"
#Software-based Signer Configuration
[[providers.softsign]]
chain_ids = ["qwoyn-1"]
key_type = "consensus"
path = "$HOME/tmkms/qwoyn/secrets/qwoyn-consensus.key"
#Validator Configuration
[[validator]]
chain_id = "qwoyn-1"
addr = "tcp://10.10.10.12:26658" #Set here IP and port of the Qwoyn node U will be using for signing blocks (port can be custom)   
secret_key = "$HOME/tmkms/qwoyn/secrets/kms-identity.key"
...
RestartSec=10
User=$USER
ExecStart=$(which tmkms) start -c $HOME/tmkms/qwoyn/tmkms.toml
LimitNOFILE=1024
[Install]
...
`INFO tmkms::keyring: [keyring:softsign] added consensus Ed25519 key: qwoynvalcons1....`    
`INFO tmkms::connection::tcp: KMS node ID: a1dbc4edb1dbb2bcd9316081bd810f57e0d`  
`ERROR tmkms::client: [qwoyn-1@tcp://<NODE IP>:26658] I/O error: Connection`  

#### LAST STEPS. Activate signing from Qwoynd node side

Find field `priv_validator_laddr = ""` at dir `$HOME/.qwoynd/config/config.toml` and edit to your Validator IP + port  
...
##### Restart Qwoynd node and check TMKMS logs   

Good logs example :  
`INFO tmkms::session: [qwoyn-1@tcp://<IP>:26658] connected to validator successfully`

`WARN tmkms::session: [qwoyn-1@tcp://<IP>:26658]: unverified validator peer ID! (ad1fc4b45ee2340bb8148d7247bf82ea780y213q)`  
`INFO tmkms::session: [qwoyn-1@tcp://<IP>:26658] signed PreCommit:<nil> at h/r/s 8825119/0/2 (0 ms)`  
`INFO tmkms::session: [qwoyn-1@tcp://<IP>:26658] signed PreVote:144665D1CE at h/r/s 8825120/0/1 (0 ms)`  
`INFO tmkms::session: [qwoyn-1@tcp://<IP>:26658] signed PreCommit:144665D1CE at h/r/s 8825120/0/2 (0 ms)`  
`INFO tmkms::session: [qwoyn-1@tcp://<IP>:26658] signed PreVote:13BF759486 at h/r/s 8825121/0/1 (0 ms)`  
