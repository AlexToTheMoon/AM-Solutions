## Remote Signing setup via TMKMS for C4E chain mainnet.
...
```
mkdir -p $HOME/tmkms/c4e
tmkms init $HOME/tmkms/c4e
```
#### Import Private key
...
```
tmkms softsign import $HOME/priv_validator_key.json $HOME/tmkms/c4e/secrets/c4e-consensus.key
```
...
```
rm -rf ~/tmkms/c4e/tmkms.toml
```
```
tee ~/tmkms/c4e/tmkms.toml << EOF
#Tendermint KMS configuration file
[[chain]]
id = "perun-1"
key_format = { type = "bech32", account_key_prefix = "c4epub", consensus_key_prefix = "c4evalcons" }
state_file = "$HOME/tmkms/c4e/state/perun-1_priv_validator_state.json"
#Software-based Signer Configuration
[[providers.softsign]]
chain_ids = ["perun-1"]
key_type = "consensus"
path = "$HOME/tmkms/c4e/secrets/c4e-consensus.key"
...
secret_key = "$HOME/tmkms/c4e/secrets/kms-identity.key"
...
RestartSec=10
User=$USER
ExecStart=$(which tmkms) start -c $HOME/tmkms/c4e/tmkms.toml
LimitNOFILE=1024
[Install]
...
`INFO tmkms::keyring: [keyring:softsign] added consensus Ed25519 key: c4evalcons1....`    
`INFO tmkms::connection::tcp: KMS node ID: a1dbc4edb1dbb2bcd9316081bd810f57e0d`  
`ERROR tmkms::client: [perun-1@tcp://<NODE IP>:26658] I/O error: Connection`  

#### LAST STEPS. Activate signing from C4E validator node side

Find field `priv_validator_laddr = ""` at dir `$HOME/.c4e-chain/config/config.toml` and edit to your Validator IP + port  
...
##### Restart C4E node and check TMKMS logs   

Good logs example :  
`INFO tmkms::session: [perun-1@tcp://<IP>:26658] connected to validator successfully`

`WARN tmkms::session: [perun-1@tcp://<IP>:26658]: unverified validator peer ID! (ad1fc4b45ee2340bb8148d7247bf82ea780y213q)`  
`INFO tmkms::session: [perun-1@tcp://<IP>:26658] signed PreCommit:<nil> at h/r/s 8825119/0/2 (0 ms)`  
`INFO tmkms::session: [perun-1@tcp://<IP>:26658] signed PreVote:144665D1CE at h/r/s 8825120/0/1 (0 ms)`  
`INFO tmkms::session: [perun-1@tcp://<IP>:26658] signed PreCommit:144665D1CE at h/r/s 8825120/0/2 (0 ms)`  
`INFO tmkms::session: [perun-1@tcp://<IP>:26658] signed PreVote:13BF759486 at h/r/s 8825121/0/1 (0 ms)`  
